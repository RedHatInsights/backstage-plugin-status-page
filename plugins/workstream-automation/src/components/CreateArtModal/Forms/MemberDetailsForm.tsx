...
export const MemberDetailsForm = (props: {
          ...
          <Tooltip
            title={
              data.role !== 'Release Train Engineer (RTE)'
                ? 'Remove member'
                : 'Please go to previous page to remove RTE'
            }
            arrow
          >
            <IconButton
  ...
  function setTableDataFn(entity: CustomUserEntity, _rte?: CustomUserEntity) {
    if (entity.metadata.uid === _rte?.metadata.uid) return;
        ...
        for (const relation of relations) {
          if (relation.type === RELATION_HAS_MEMBER) {
            catalogApi.getEntityByRef(relation.targetRef).then(res => {
              if (res) {
                setTableDataFn(res as CustomUserEntity, rte);
              }
            });
          }
        }
      } else setTableDataFn(entity, rte);
    }
  }
