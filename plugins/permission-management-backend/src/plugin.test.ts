...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [permissionManagementPlugin],
    });

    await request(server).get('/api/permission-management/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/permission-management/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/permission-management/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/permission-management/todos/${createdTodoItem.id}`)
