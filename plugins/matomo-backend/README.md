...
```bash
yarn add --cwd packages/backend  @compass/backstage-plugin-matomo-backend
```

2. Then create a new file `packages/backend/src/plugins/matomo.ts`, and add the following

```ts
import { createRouter } from '@compass/backstage-plugin-matomo-backend';
